...
## Next up (active)
- [ ] **OOP primer, part 3 of 3 (Session 23 → 25):** functions and decorators both done and verified (registering vs. wrapping decorators, parametrized 3-level decorators). OOP is the last piece of his original 3-part request. **Explicitly chosen as next session's objective (2026-08-02, at Session 25's close)** — do this before the `api.py` test below.
- [ ] **Tests for `api.py`, part 2 (Session 23):** golden-path test (`test_query_golden_path`, `TestClient` + `monkeypatch`) shipped and committed (`81cde40`). Still missing: a test forcing an exception through `monkeypatch` and asserting the client gets the structured JSON error + 500 (the exception-handler path, still zero coverage). Unblocked as of Session 25 (both primer prerequisites done) but queued **after** OOP per his explicit choice above.
