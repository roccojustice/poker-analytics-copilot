...
## Next up (active)
- [ ] **Tests for `api.py` (Session 22):** zero test coverage today. New territory — decide whether to test at the HTTP layer (`fastapi.testclient.TestClient`/`httpx`, exercising the real request/response cycle including the exception handler) or mock straight through to `query()`'s internals (`monkeypatch`, same pattern as `tests/test_db.py`). Decide the layer before writing the first test.
