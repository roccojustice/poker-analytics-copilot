...
## Next up (active)
- [ ] **Exception handler in `api.py` (Session 19):** unhandled exceptions (`ValueError`, `KeyError`, etc.) currently return FastAPI's raw `500`. Needs a catch-all handler returning a structured JSON error — defense-in-depth alongside the group_by validation. **Before/alongside this:** do a live `uvicorn` smoke test of `/query` — no test file covers `api.py`, and its call into `run_query()` (which gained the `group_by` guard) was never verified live after Session 21's merge.
