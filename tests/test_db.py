...
def test_run_filter_query(monkeypatch):
    ...
    assert "hrt.total_p_raises = %(total_raises_1)s" in result["query"]
    assert result["params"] == {"total_raises_1": 1, "id_player": 42, "limit": 10}
...
def test_run_filter_query_with_since_date(monkeypatch):
    ...
    assert result["params"] == {"total_raises_1": 1, "id_player": 42, "since_date": "2023-01-01"}
...
def test_run_filter_query_clause_order_with_since_date_and_limit(monkeypatch):
    ...
    assert result["params"] == {"total_raises_1": 1, "id_player": 42, "since_date": "2023-01-01", "limit": 10}