...
## Next up (active)
- [ ] **Tests for `api.py`, part 2 (Session 23):** golden-path test (`test_query_golden_path`, `TestClient` + `monkeypatch`) shipped and committed (`81cde40`). Still missing: a test forcing an exception through `monkeypatch` and asserting the client gets the structured JSON error + 500 (the exception-handler path, still zero coverage). Blocked behind the decorators primer (see below) — functions half of the prerequisite is done as of Session 24.
- [ ] **Decorators primer, part 2 of 3 (Session 24):** functions foundation (first-class objects, higher-order functions, closures) done and verified. Next: decorators — start by reading `@app.exception_handler` in his own `api.py` with the two-pass reading technique confirmed Session 24, before writing one. OOP still after that. Prerequisite for resuming the `api.py` exception-handler test above.
