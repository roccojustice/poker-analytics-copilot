...
## Next up (active)
- [ ] **Tests for `api.py`, part 2 (Session 23):** golden-path test (`test_query_golden_path`, `TestClient` + `monkeypatch`) shipped and committed (`81cde40`). Still missing: a test forcing an exception through `monkeypatch` and asserting the client gets the structured JSON error + 500 (the exception-handler path, still zero coverage). Fully unblocked as of Session 26 (all 3 primer prerequisites done) — explicitly deferred to next session by his own choice.
